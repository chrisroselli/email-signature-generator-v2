    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className="border-[#47403d]/20 text-[#47403d] hover:bg-primary/10"
        >
          Change Icon
        </Button>
      </DialogTrigger>
      <DialogContent className="border-primary/20 sm:max-w-md">
        ...
        <Tabs defaultValue="default" value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="default" className="mt-4">
            <div className="grid grid-cols-3 gap-4">
              {defaultIcons[platform as keyof typeof defaultIcons]?.map((icon, index) => (
                <div
                  key={index}
                  className="flex cursor-pointer items-center justify-center rounded-md border border-[#47403d]/20 p-4 hover:bg-primary/10"
                  onClick={() => handleIconSelect(icon)}
                >
                  <img
                    src={icon || '/placeholder.svg'}
                    alt={`${platform} icon ${index + 1}`}
                    width="32"
                    height="32"
                  />
                </div>
          ...
          <TabsContent value="custom" className="mt-4">
            <div className="flex flex-col items-center justify-center rounded-md border-2 border-dashed border-[#47403d]/20 p-6">
              <Upload className="mb-2 h-10 w-10 text-[#47403d]/50" />
